...
def project_edge_on(points, theta_deg, phi_deg):
    theta = np.deg2rad(theta_deg)
    phi = np.deg2rad(phi_deg)
    r = 1
    # p = r * np.sin(theta) * np.cos(phi)
    # q = r * np.sin(theta) * np.sin(phi)
    # r = r* np.cos(theta)

    # v = np.array([
    #     [np.cos(theta), 0, -np.sin(theta), (-r * np.cos(theta)) + (r * np.cos(theta))],
    #     [0, 1, 0, -1 * q],
    #     [np.sin(theta), 0, np.cos(theta), -1 * p * np.sin(theta) - (r * np.cos(theta))]
    # ])
    ...
    rotMat = np.matmul(xAxis, yAxis)
    newArr = np.matmul(points, rotMat)
    return newArr
