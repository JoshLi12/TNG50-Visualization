...
#            print(idx, np.log10(Mdom_all[i]),snapdom_all[i],snapdom_merged[i],merged_all[i],'not yet merged')
    ...
    for i in range(1,11):
        ...
        galaxy_output = os.path.join(bp_local, "sample_models", str(idx))
        ...
        angles = []
        fname_edge = galaxy_output+'/cutout_'+str(subfindID0)+'_edge' + '.hdf5'
        fname_face = galaxy_output+'/cutout_'+str(subfindID0)+'_face' + '.hdf5'
        angles.append(fname_edge)
        angles.append(fname_face)
        for fname in angles:
            # print(fname)
        ...
        #        bovera[i] = 10**fmin(avdens)/10**fmaj(avdens)
            ...
            os.makedirs(os.path.join(galaxy_output, "images"), exist_ok=True)
            plt.savefig(galaxy_output+'/images/'+str(idx)+'_'+fname[len(fname)-9:len(fname)-5]+'.png', format='png',dpi=200)
